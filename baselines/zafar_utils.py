    ...
    if apply_accuracy_constraint == 0:  # its not the reverse problem, just train w with cross cov constraints
        ...
        if sep_constraint:
            for i in range(0, len(predicted_labels)):
                if predicted_labels[i] == 1.0 and x_control.loc[i, sensitive_attrs].any() == 1.0:
                    c = ({'type': 'ineq', 'fun': constraint_protected_people, 'args': (x.loc[i, :], y[i])})
                    constraints.append(c)
                else:
                    c = ({'type': 'ineq', 'fun': constraint_unprotected_people,
                          'args': (i, unconstrained_loss_arr[i], x.loc[i, :], y[i])})
                    constraints.append(c)
